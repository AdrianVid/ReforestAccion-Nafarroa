...
const { User, Roles } = require("../modelos/User");
const { authRole, checkToken } = require("../middleware");
...
//Añadir plantacion
UserRoute.put("/find/plantaciones", async (req, res) => {
  try {
    const { id } = req.user;
    ...
    if (!plantaciones) {
      return res.status(400).json({
        succes: false,
        message: `Tienes que introducir una plantación`,
      });
    }
...
UserRoute.put("/find/apadrinados", async (req, res) => {
  try {
...
UserRoute.put("/find/apadrinados/delete", async (req, res) => {
  try {
    const { plantacion, arbol, cantidad } = req.body;
    const { id } = req.user;
...
UserRoute.delete("/find/delete", async (req, res) => {
  try {
    const { id } = req.user;
